...
%>
	...
	<table id="display1">
		...
		<tr>
			...
			<p><a href="./products.jsp">상품 목록 돌아가기</a>
			</td>
		...
		cnt ++; 
		if (rs.getString("p_id").equals(p_id)){
			
	%>

			<td id="td_style" width="15%" align=center><img id="img_style"
				src="./MacBook/<%=rs.getString("p_fileName")%>">

				<h3>상품 : <%=rs.getString("p_name")%></h3>
				<p>가격 : <%=rs.getString("p_unitPrice")%>원
				<p>설명 : <%=rs.getString("p_descript") %>
				<p>범주 : <%=rs.getString("p_category") %>
				<p>제조사 : <%=rs.getString("p_manufact") %>
				<p>제고량 : <%=rs.getString("p_unitsStock") %>개
				<p>제품상태 : <%=rs.getString("p_descript") %>
			<%
		}
